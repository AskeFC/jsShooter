...
socket.on('newPositions', function(data) {
	...
	for (let i = 0, iEnd = data.powerups.length; i < iEnd; ++i) {
		ctx.strokeStyle = '#00F';
		ctx.beginPath();
	...
	for (let i = 0, iEnd = data.bullets.length; i < iEnd; ++i) {
		ctx.fillStyle = '#000';
		ctx.beginPath();
		let renderSize = 2;
	...
	for (let i = 0, iEnd = data.blocks.length; i < iEnd; ++i) {
		ctx.fillStyle = '#000';
	...
	for (let i = 0, iEnd = data.attackers.length; i < iEnd; ++i) {
		ctx.fillStyle = '#00F';
	...
	for (let i = 0, iEnd = data.shooters.length; i < iEnd; ++i) {
		ctx.fillStyle = '#F00';
